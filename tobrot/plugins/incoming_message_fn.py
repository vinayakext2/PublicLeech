...
async def leech_commandi_f(client, message):
    ...
    if dl_url is not None:
        await m_.edit_text("Digging up links 🤯")
        ...
        if "_" in m_sgra:
            await m_.edit_text("Downloading for you and Wasting my data 🥵")
...
async def incoming_youtube_dl_f(client, message):
    """ /ytdl command """
    i_m_sefg = await message.reply_text("Cooking... Wait", quote=True)
    # LOGGER.info(message)
    ...
    LOGGER.info(cf_name)
    if dl_url is not None:
        await i_m_sefg.edit_text("Digging up links 🤯")
        ...
        await i_m_sefg.edit_text(
            "Sed Lyf Boy 🥲 An Error Jumping In. \nPlease read /help \n"
